...
extr_MicrobeJMESH <- function(dataloc, sep = ",", extracols) {
  ...
  lengthlist <- lapply(seq_len(length(bblist)), function(x) max(bblist[[x]]))
  widthlist <- lapply(seq_len(length(bblist)), function(x) min(bblist[[x]]))
...
extr_MicrobeJ <- function(dataloc,
  ...
  if (!missing(objectloc)) {
    ...
    objectsout <- objectsout %>%
      ...
      dplyr::mutate(obarea = sp::Polygon(cbind(.data$ob_x, .data$ob_y))@area) %>%
      dplyr::ungroup()
...
extr_ObjectJ <- function(dataloc,
  ...
  oj2 <- reshape2::melt(oj,
    measure.vars = boundingBoxX, value.name = "Xum",
    variable.name = "index_X"
  )

  oj2 <- reshape2::melt(oj2,
    measure.vars = boundingBoxY, value.name = "Yum",
