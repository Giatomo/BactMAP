...
combineDataframes <- function(listofdataframes, listofconditions, listofchannels, output = "finalframe") {
  ...
  out <- switch(output,
    "all" = list("finalframe" = finalframe, "originaldata" = listofdataframes),
    "finalframe" = list("finalframe" = finalframe),
    "originaldata" = list("originaldata" = listofdataframes)
  )
...
plotOverlay <- function(meshdata,
  ...
  # check type input
  if (type != "all" & type != "projection" & type != "histogram" & type != "length" & type != "width") {
    type <- readline("Please give type of plot: 'histogram', 'length', 'width', 'projection', or 'all' and press enter to confirm.")
    if (type != "all" & type != "projection" & type != "histogram" & type != "length" & type != "width") {
      stop("Plot type not recognized")
    }
  }
  # check by input
  if (by != "both" & by != "channel" & by != "condition") {
    by <- readline("Please give the correct factor for seperating the plots: 'channel', 'condition', or 'both' and press enter to confirm.")
    if (by != "both" & by != "channel" & by != "condition") {
      stop("Factor 'by' not recognized.")
    }
  }
  ...
  if (missing(objectdata) != T) {
    if ("ob_out_x" %in% colnames(objectdata) != T) {
      if ("l" %in% colnames(objectdata) != T) {
        stop("No object outlines correlated to cell outlines found ('ob_out_x'/'ob_out_y' or 'l'/'d'). Please correlate outlines to cells using 'spotsInBox' before entering to this function")
      }
      if (missing(mag)) {
        stop("No magnification correction value ('mag') found. Add value 'mag' in function or add columns 'ob_out_y' & 'ob_out_x' to objectdata")
      }
      if ("l" %in% colnames(objectdata) == T) {
  ...
  if (missing(spotdata) != T) {
    if ("Lmid" %in% colnames(spotdata) != T) {
      if ("l" %in% colnames(spotdata) != T) {
        stop("Spots are not related to any cells yet. Please use 'spotsInBox' to add cell information and add pixel-to-micron converter 'mag' in this function.")
      }
      if (missing(mag)) {
        stop("Please add pixel-to-micron converter 'mag' to the function to calculate the spot ggplot2::coordinates in micron.")
      }
  ...
  if (type == "histogram" | type == "all") {
    ...
    if (missing(meshdata) != T) {
      ...
      his_mesh <- ggplot2::ggplot(meshdata) +
        ggplot2::geom_density(ggplot2::aes_string(x = "quant_by"), fill = meshcolor, color = histogram_outline) +
        ggplot2::theme_minimal()
      if (by != "channel") {
    ...
    if (missing(objectdata) != T) {
      ...
      if (by == "channel" | by == "both") {
        his_obj <- ggplot2::ggplot(objectdata) +
          ggplot2::geom_density(ggplot2::aes_string(x = "Lmid", fill = "channel"), color = histogram_outline, alpha = 0.5) +
          ggplot2::theme_minimal()
      }
      if (by == "condition") {
        his_obj <- ggplot2::ggplot(objectdata) +
