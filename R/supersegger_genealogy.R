...
extr_SuperSeggerClist <- function(matfile, trim.orphans = TRUE, cellList = FALSE) {
  if (!requireNamespace("R.matlab", quietly = TRUE)) {
    ...
    if (inp %in% c("y", "Y")) {
      utils::install.packages("R.matlab")
    } else {
      stop("Canceled")
    }
  }
...
plotTreeBasic <- function(phylo, extradata, yscalechange = FALSE, showClade = FALSE, layout = "rectangular", ydata, cellNumber, open.angle, linesize = 1, linecolor = "black", lines = TRUE, colors = FALSE) {
  ...
  if (showClade == TRUE) {
    if (missing(cellNumber)) {
      stop("cellNumber missing. Please state the ancestor cell you want to follow to show it's clade.")
    }
  ...
  if (showClade != TRUE) {
    gP <- ggtree::ggtree(phylo, layout = layout, open.angle = open.angle, size = linesize, color = linecolor)
  }
  ...
  if (yscalechange == TRUE) {
    if (missing(ydata)) {
      stop("Variable 'ydata' missing. Don't know what values to put on the y axis. Please give the column name of your data as 'ydata' in the function")
    }
...
getphylolist_SupSeg <- function(CLT, prep = FALSE) {
  ...
  for (n in unique(CLT$root[!is.na(CLT$root)])) {
    onephyl <- CLT[CLT$root == n & , ]
    if (nrow(onephyl) > 1) {
      z <- z + 1
      Ntip <- length(onephyl$cell[onephyl$cell %in% onephyl$parent != T])
      Ntip <- sum(onephyl$cell %!in% onephyl$parent)
      if (Ntip > 1) {
